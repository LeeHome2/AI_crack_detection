...
파이프라인: 입력(촬영/업로드) -> 탐지 -> Feature -> (RAG + Rule) -> 보고서 -> 화면
- 모바일 웹: st.camera_input(폰 카메라 직촬) + st.file_uploader(원본 업로드)
- 결과는 세로 스택 레이아웃(폰 화면 대응). 모델/API 없어도 동작(안내 표시).
"""
import numpy as np
import cv2
...
# 촬영·업로드 모두 getvalue()로 바이트 획득 (read()와 달리 재호출에도 안전)
...
st.image(cv2.cvtColor(vis, cv2.COLOR_BGR2RGB), use_container_width=True,
         caption=f"탐지된 균열 {feat.crack_count}개")
