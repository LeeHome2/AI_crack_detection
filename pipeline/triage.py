...
- retake_far : 건물 전체·외벽 전경처럼 구조물이 멀리/작게 찍혀 개별 결함의 폭·형태·경계를
               근접 판별할 수 없는 경우. 결함(의심 포함)이 어렴풋이 보여도, 화면에서 매우 작게만
               보이거나 건물 한 동이 통째로 들어오는 전경이면 반드시 retake_far.
- retake_blur: 초점 흐림·흔들림으로 결함 경계가 뭉개짐
- not_crack  : 시설물 결함 점검 대상 사진이 아님(사람·풍경·문서 등)
- ok         : 벽면·기둥·바닥의 일부가 화면을 충분히 채운 근접~중거리 촬영으로, 결함(또는 결함
               의심)의 형태를 판별할 수 있음 → 분석 진행. (근접성이 애매하면 ok보다 retake_far)"""
