...
> ⚠️ 아래 ⬇ 표시 파일은 클라우드 프록시(robots/SSL)에서 자동 다운로드가 막혀 **직접 받아 올려야** 함.
> 저장 규칙: 아래 "저장 파일명" 그대로 `knowledge/sources/`에 저장하면 매니페스트와 일치.

---

## A. 복합 결함(2차 MVP) 핵심 출처 — ⬇ 직접 다운로드 요망
...
| S3 | 건축물 안전점검 및 정밀안전진단 세부지침 | 건설교통부/시설안전공단 · 2003 | https://www.codil.or.kr/filebank/moct2014/200402/MOCT737_2.PDF | `세부지침_2003_건축물_MOCT737.pdf` | 03·08·12·13·14·15 |
...
| S11 | 콘크리트균열보수 전문시방서 (LHCS 14 20 10 25:2020) | 한국토지주택공사(LH)/KCSC | https://www.kcsc.re.kr | `LH_균열보수_시방서_LHCS14201025.pdf` | 11 |
| S12 | 공동주택 하자의 조사·보수비용 산정 및 하자판정 기준 (고시 2016-1048) | 국토교통부 | https://www.law.go.kr | `국토부고시_2016-1048_하자판정기준.pdf` | 10 |
...
3. 저장 후 커밋: `git add knowledge/sources/ && git commit -m "docs: RAG 원문 출처 아카이브" && git push`
4. 대용량 PDF는 git에 직접 커밋하거나(수십 MB), 부담되면 이 매니페스트만 커밋하고 파일은 로컬 보관해도 됨(링크로 추적 가능).
